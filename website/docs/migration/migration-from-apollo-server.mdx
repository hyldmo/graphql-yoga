...
For example, if you are using **Express**, you should remove `const server = createServer(yoga) server.listen(4000)` from the code above and replace `server.applyMiddleware({ app })` with the route as in [Express Integration section](/docs/integrations/integration-with-express)
